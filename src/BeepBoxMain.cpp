...
void showVersion(int silentMode)
{
  if (silentMode == 0)
  {
    ...
    std::cerr << "*   -- BeepBox v1.0.0 [20220427] --                        *" << std::endl;    
...
int main(int argc, char** argv)
{
  ...
  auto printHelp = [&]()
  {
    ...
    showVersion(0);
    std::cerr << "USAGE:\n";
    std::cerr << "  BeepBox -k <KEY> -o <OUTPUT.wav> [options]\n\n";
    std::cerr << "NOTAS:\n";
    std::cerr << "  - One beep lasts at least 2.3s; start + 2.3 <= duration\n";
    std::cerr << "  - Interval sets repetition; if only one beep fits it is ignored with a warning\n";
    std::cerr << "  - Mixer options (-f/-x/-p) require an input WAV (--file)\n\n";
    std::cerr << "REQUIRED:\n";
    std::cerr << "  -k, --key <KEY>                 5 chars base32 [0-9 a-v] (required)\n";
    std::cerr << "  -o, --output <PATH>             Output WAV (required)\n\n";
    std::cerr << "OPTIONAL (generation):\n";
    std::cerr << "  -m, --mode <0|1|2|3>            0 audible | 1 hidden | 2 non-audible | 3 custom (default: 2)\n";
    std::cerr << "  -d, --duration <seconds>        Total duration (min 2.3). Default: 2.3\n";
    std::cerr << "  -s, --start <seconds>           First beep start (>=0). Default: 0.0\n";
    std::cerr << "  -i, --interval <seconds>        Repetition (min 2.3). Default: 2.3; ignored if only 1 beep fits\n";
    std::cerr << "  -r, --samplerate <Hz>           44100 or 48000 when no --file. Default: 44100\n";
    std::cerr << "  -n, --dry-run                   Validate and show plan; no audio generated\n\n";
    std::cerr << "MIXER (requires --file):\n";
    std::cerr << "  -f, --file <PATH>               Input WAV (44.1k/48k PCM16)\n";
    std::cerr << "  -x, --mixmode <0|1|2>           0 DefaultLevel | 1 GlobalLevel | 2 DynamicLevel\n";
    std::cerr << "  -p, --volumeprogram <dB>        Program gain (default: 0)\n";
    std::cerr << "  -l, --loudnessstatistics <0|1>  Loudness stats (default: 0)\n\n";
    std::cerr << "CUSTOM MODE (mode=3 only):\n";
    std::cerr << "  --bf, --basefreq <Hz>           Base frequency (default: 12000)\n";
    std::cerr << "  --ts, --tonesseparation <int>   Tone separation (>=1). Default: 1\n\n";
    std::cerr << "ADVANCED (synthesis):\n";
    std::cerr << "  -sm, --synthmode <0|1>          0 off | 1 r2d2 (clamped). Default: 0\n";
    std::cerr << "  -sv, --synthvolume <float>      Synth volume dB relative to beeps (-60..12). Default: 0\n";
    std::cerr << "  -v, --volumebeeps <dB>          Beeps gain (-60..12). Default: -3\n\n";
    std::cerr << "UTILITIES:\n";
    std::cerr << "  -h, --help                      Show this help\n\n";
    std::cerr << "EXAMPLES:\n";
    std::cerr << "  BeepBox -k 0abc1 -o beeps.wav\n";
    std::cerr << "  BeepBox -k 0abc1 -f input.wav -o mixed.wav -x 1 -i 4\n";
    std::cerr << "  BeepBox -k 0abc1 -m 3 -bf 15000 -ts 10 -o custom.wav\n";
    std::cerr << "  BeepBox -n -k 0abc1 -d 5 -o plan.wav\n";
  };
