...
int main(int argc, char** argv)
{
  ...
  auto printHelp = [&]()
  {
    ...
    std::cerr << "  -d, --duration <float>     Duration seconds (>=2.3; start+2.3 <= duration). Default 2.3\n";
    std::cerr << "  -i, --interval <float>     Interval between beeps (>=2.3). Default 2.3; ignored if solo 1 beep\n";
    std::cerr << "  -s, --start <float>        Start time for first beep (>=0). Default 0\n";
    std::cerr << "  -x, --mixmode <int>        0 DefaultLevel | 1 GlobalLevel | 2 DynamicLevel (requires --file)\n";
    ...
    std::cerr << "  -n, --dry-run              Validate and show plan; no audio generated or files written\n";
    std::cerr << "\nExamples:\n";
