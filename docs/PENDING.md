...
* 🏷️ **Tipo**: fix
* 🧭 **Trigger**: smoke E2E de v0.1.0 deploy en dev + prod. Tanto `BEEPBOX_TEST_KEY_DEV=bk_PkvVhXmkqJncUaoCf-kPvQ` como `BEEPBOX_TEST_KEY_PROD=bk_6MxoDyc1NdhodEYgdcT2xw` (minteadas en BEE-1802 para SDK E2E suites) devuelven `HTTP 403 {"error":"Invalid API key"}` contra `/v1/encode` y `/v1/decode` en sus respectivos environments. El binary procesa el request y rechaza vía `requireAuth` → el problema es validator-side, no servidor.
